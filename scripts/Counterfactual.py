    ...
    def __decode(self, data, encoders):
        ...
        for col, encoder in encoders.items():
            ...
            # --- OneHotEncoder ---
                ...
                if all(c in data.columns for c in ohe_cols):
                    ...
                    original_categories = encoder.inverse_transform(data[ohe_cols]).ravel()
                    ...
                    data[col] = pd.Series(original_categories, index=data.index) 
                else:
                    missing_ohe_cols = [c for c in ohe_cols if c not in data.columns]
            ...
            # --- CountVectorizer ---
                ...
                if all(c in data.columns for c in cv_feature_names):
                    # 'transformed' será una lista de arrays de strings (tokens)
                    transformed_tokens_list = encoder.inverse_transform(data[cv_feature_names])
                    ...
                    data = data.drop(columns=cv_feature_names)
                    # Añadir la columna 'col' (nombre original de la característica de texto) con el texto reconstruido.
                    data[col] = pd.Series(reconstructed_text, index=data.index)
                else:
                    missing_cv_cols = [c for c in cv_feature_names if c not in data.columns]
        ...
        if self.target in cols:
            ...
            target_idx = cols.index(self.target)
